...
abstract class Type
{
    ...
    {
        throw new \CallerException('Type should never be instantiated.  All methods and variables are publically statically accessible.');
    }
    ...
    private static function CastObjectTo($objItem, $strType)
    {
        ...
        throw new InvalidCastException(sprintf('Unable to cast %s object to %s', $objReflection->getName(), $strType));
    }
    ...
    private static function CastValueTo($mixItem, $strNewType)
    {
        ...
        switch (static::TypeFromDoc($strNewType)) {
            ...
            case static::Integer:
                if ($strOriginalType == static::Boolean) {
                    throw new InvalidCastException(sprintf('Unable to cast %s value to %s: %s', $strOriginalType, $strNewType, $mixItem));
                }
                if (strlen($mixItem) == 0) {
                    return null;
                }
                ...
                // any other scenario is an invalid cast
                throw new InvalidCastException(sprintf('Unable to cast %s value to %s: %s', $strOriginalType, $strNewType, $mixItem));
            case static::Float:
                if ($strOriginalType == static::Boolean) {
                    throw new InvalidCastException(sprintf('Unable to cast %s value to %s: %s', $strOriginalType, $strNewType, $mixItem));
                ...
                if (!is_numeric($mixItem)) {
                    throw new InvalidCastException(sprintf('Invalid float: %s', $mixItem));
                }
                // Check to make sure the value hasn't changed significantly.
                $fltItem = $mixItem;
            ...
            default:
                throw new InvalidCastException(sprintf('Unable to cast %s value to unknown type %s', $strOriginalType, $strNewType));
        }
    }
    ...
    private static function CastArrayTo($arrItem, $strType)
    {
        if ($strType == static::ArrayType) {
            ...
            throw new InvalidCastException(sprintf('Unable to cast Array to %s', $strType));
        }
    }
    ...
    public final static function Cast($mixItem, $strType)
    {
        switch (static::$intBehaviour) {
            ...
            case static::CastOnly:
                throw new CallerException("static::CastOnly handling not yet implemented");
                break;
            case static::CheckOnly:
                throw new CallerException("static::CheckOnly handling not yet implemented");
                break;
        ...
        switch ($strPhpType) {
            ...
            case static::Resource:
                ...
                throw new InvalidCastException('Resources cannot be cast');

            default:
                // Could not determine type
                throw new InvalidCastException(sprintf('Unable to determine type of item to be cast: %s', $mixItem));
        }
    }
    ...
    public final static function Constant($strType)
    {
        switch ($strType) {
            ...
            default:
                // Could not determine type
                throw new InvalidCastException(sprintf('Unable to determine type of item to lookup its constant: %s', $strType));
        }
    }
        ...
        switch (strtolower($strType)) {
            ...
            default:
                try {
                    ...
                    throw new InvalidCastException(sprintf('Unable to determine type of item from PHPDoc Comment to lookup its static or Class: %s', $strType));
                }
        }
    }
